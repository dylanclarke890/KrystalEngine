#include "Krystal.Text/Count/CountAsTranscoded.hpp"
...
namespace Krys::Tests
{
  ...
  TEST_CASE("CountAsTranscoded(utf8)", "[Text][CountAsTranscoded]")
  {
    auto result0 = CountAsTranscoded(u32_basic_source_character_set, utf8);
    ...
    auto result1 = CountAsTranscoded(u32_unicode_sequence_truth_native_endian, utf8);
  ...
  TEST_CASE("CountAsTranscoded(utf16)", "[Text][CountAsTranscoded]")
  {
    auto result0 = CountAsTranscoded(u32_basic_source_character_set, utf16);
    std::size_t expected0 = std::size(u16_basic_source_character_set);
    REQUIRE_FALSE(result0.ErrorsWereHandled());
    REQUIRE(result0.Count == expected0);

    auto result1 = CountAsTranscoded(u32_unicode_sequence_truth_native_endian, utf16);
    std::size_t expected1 = std::size(u16_unicode_sequence_truth_native_endian);
    REQUIRE_FALSE(result1.ErrorsWereHandled());
    REQUIRE(result1.Count == expected1);
  }

  TEST_CASE("CountAsTranscoded(utf32)", "[Text][CountAsTranscoded]")
  {
    auto result0 = CountAsTranscoded(u32_basic_source_character_set, utf32);
    std::size_t expected0 = std::size(u32_basic_source_character_set);
    REQUIRE_FALSE(result0.ErrorsWereHandled());
    REQUIRE(result0.Count == expected0);

    auto result1 = CountAsTranscoded(u32_unicode_sequence_truth_native_endian, utf32);
