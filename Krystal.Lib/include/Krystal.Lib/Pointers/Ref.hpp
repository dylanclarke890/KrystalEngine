...
namespace WTF
{
  ...
  public:
    ...
    Ref(T &object) : m_ptr(&RefDerefTraits::ref(object))
    {
    }

    Ref(const Ref &other) : m_ptr(&RefDerefTraits::ref(other.get()))
    {
    }

    template <typename X, typename Y>
    Ref(const Ref<X, Y> &other) : m_ptr(&RefDerefTraits::ref(other.get()))
    {
    }

    Ref(Ref &&other) : m_ptr(&other.leakRef())
    {
      ASSERT(m_ptr);
    }

    template <typename X, typename Y>
    Ref(Ref<X, Y> &&other) : m_ptr(&other.leakRef())
    {
      ASSERT(m_ptr);
    }
    ...
    {
      return PtrTraits::isHashTableDeletedValue(m_ptr);
    }
    ...
    {
      return m_ptr == hashTableEmptyValue();
    }
    ...
    const T *ptrAllowingHashTableEmptyValue() const LIFETIME_BOUND
    {
      ASSERT(m_ptr || isHashTableEmptyValue());
      return PtrTraits::unwrap(m_ptr);
    }
    T *ptrAllowingHashTableEmptyValue() LIFETIME_BOUND
    {
      ASSERT(m_ptr || isHashTableEmptyValue());
      return PtrTraits::unwrap(m_ptr);
    }
    ...
    T *ptr() const LIFETIME_BOUND RETURNS_NONNULL
    {
      ASSERT(m_ptr);
      return PtrTraits::unwrap(m_ptr);
    }
    T *unsafePtr() const RETURNS_NONNULL
    {
      ASSERT(m_ptr);
      return PtrTraits::unwrap(m_ptr);
    } // FIXME: Replace with ptr() then remove.
    ...
    template <typename X, typename Y, typename Z>
    WARN_UNUSED_RETURN Ref<T, PtrTraits, RefDerefTraits> replace(Ref<X, Y, Z> &&);

    // The following function is deprecated.
    Ref copyRef() && = delete;
    WARN_UNUSED_RETURN Ref copyRef() const &
    {
      return Ref(*m_ptr);
    }
  ...
  private:
    ...
    Ref(T &object, AdoptTag) : m_ptr(&object)
    {
    }
  ...
  template <typename X, typename _PtrTraits = RawPtrTraits<X>,
            typename RefDerefTraits = DefaultRefDerefTraits<X>, typename Y, typename _OtherPtrTraits,
            typename OtherRefDerefTraits>
  ALWAYS_INLINE Ref<X, _PtrTraits, RefDerefTraits>
  ...
  inline Ref<match_constness_t<Source, Target>>
  ...
  template <typename Target, typename Source, typename PtrTraits, typename RefDerefTraits>
  inline RefPtr<match_constness_t<Source, Target>>
