...
#include "Krystal.Text/TranscodeIterator.hpp"
#include <string_view>
...
namespace Krys
{
  ...
  /// EncodeOne . It may also call `TranscodeOne` to bypass having to do the round-trip through
  ...
  class TranscodeView : public Krys::Ranges::ViewBase
  {
  public:
    /// @brief The iterator type for this view.
    using iterator = TranscodeIterator<TFromEncoding, TToEncoding, TRange, TFromErrorHandler, TToErrorHandler,
                                       TFromState, TToState>;

    /// @brief The sentinel type for this view.
    using sentinel = TTranscodeSentinel;
    ...
    /// @brief Constructs a TranscodeView from the underlying range.
    ///
    /// @param[in] range The input range to wrap and iterate over.
    constexpr TranscodeView(range_type range) noexcept : TranscodeView(std::move(range), to_encoding_type {})
    {
    }
    ...
    constexpr TranscodeView(range_type range, to_encoding_type toEncoding) noexcept
        : TranscodeView(std::move(range), from_encoding_type {}, std::move(toEncoding))
    {
    }
    ...
    /// @param[in] toEncoding The encoding object to call `EncodeOne` or equivalent functionality on.
    constexpr TranscodeView(range_type range, from_encoding_type fromEncoding,
                            to_encoding_type toEncoding) noexcept
        : TranscodeView(std::move(range), std::move(fromEncoding), std::move(toEncoding),
    ...
    constexpr TranscodeView(range_type range, from_encoding_type fromEncoding, to_encoding_type toEncoding,
...
namespace std::ranges
{
  ...
  constexpr inline bool enable_borrowed_range<Krys::TranscodeView<
