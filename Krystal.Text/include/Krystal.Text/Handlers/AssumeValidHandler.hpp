...
namespace Krys::Text::Handlers
{
  ...
  public:
    ...
    /// it cannot ignore it.
    using assume_valid = IntegralConstant<bool, TrapsOnInvocation>;

    constexpr static inline const bool decode_always_ok = true;

    constexpr static inline const bool encode_always_ok = true;
                                             ...
                                             const TOutputProgress &) const noexcept(!TrapsOnInvocation)
    {
      ...
      {
        assert(false
               && "The handler was called but the text was assumed to be valid, this can lead to undefined "
                  "behaviour!");
      }
