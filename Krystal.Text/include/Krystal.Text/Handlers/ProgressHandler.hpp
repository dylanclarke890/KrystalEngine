...
namespace Krys::Text::Handlers
{
  ...
  template <typename THandler>
  concept IsProgressHandler = IsProgressHandlerSpecialization<remove_cvref_t<THandler>>::value;
  ...
  private:
    ...
    constexpr inline static bool TIsProgressHandler =
                                                            ...
                                                            std::forward<TOutputProgress>(outputProgress))))
    {
      if constexpr (TIsProgressHandler)
      {
        ...
        if constexpr (Krys::IsSpecializationOf<remove_cvref_t<TResult>, DecodeResult>)
        {
          ...
          std::ranges::copy_n(std::ranges::cbegin(inputProgress), nonConstSelf._codeUnitsSize,
                                          nonConstSelf._codeUnits.data());
          nonConstSelf._codePointsSize = std::ranges::size(outputProgress);
          std::ranges::copy_n(std::ranges::cbegin(outputProgress), nonConstSelf._codePointsSize,
                                          nonConstSelf._codePoints.data());
        }
        else
        {
          nonConstSelf._codePointsSize = std::ranges::size(inputProgress);
          std::ranges::copy_n(std::ranges::cbegin(inputProgress), nonConstSelf._codePointsSize,
                                          nonConstSelf._codePoints.data());
          nonConstSelf._codeUnitsSize = std::ranges::size(outputProgress);
          std::ranges::copy_n(std::ranges::cbegin(outputProgress), nonConstSelf._codeUnitsSize,
                                          nonConstSelf._codeUnits.data());
        }
      }
