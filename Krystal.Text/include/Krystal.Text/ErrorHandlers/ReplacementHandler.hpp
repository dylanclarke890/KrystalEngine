...
namespace Krys::Handlers
{
  namespace Impl
  {
    ...
    constexpr TResult &&WriteStaticCodePointsDirect(const TEncoding &encoding, TResult &&result) noexcept
    {
      ...
      {
        constexpr TInputCodePoint replacements[1] = {
          static_cast<TInputCodePoint>(Krys::Impl::Unicode::Replacement)};
    ...
    constexpr TResult &&WriteStaticCodeUnitsDirect(const TEncoding &encoding, TResult &&result) noexcept
    {
      ...
      {
        constexpr TInputCodeUnit replacements[1] = {
          static_cast<TInputCodeUnit>(Krys::Impl::Unicode::Replacement)};
        return Impl::WriteDirect(encoding, replacements, std::forward<TResult>(result));
      }
      else
      {
      ...
      code_point_t<TEncoding> (&replacementCodePoints)[MaxCodePoints<TEncoding>])
    {
      ...
      {
        replacementCodePoints[0] = static_cast<TInputCodePoint>(Krys::Impl::Unicode::Replacement);
        return 1;
      }
      else
      {
                                    ...
                                    code_unit_t<TEncoding> (&replacementCodeUnits)[MaxCodeUnits<TEncoding>])
    {
      ...
      else if constexpr (IsUnicodeCodePoint<TInputCodeUnit>)
      {
        replacementCodeUnits[0] = static_cast<TInputCodeUnit>(Krys::Impl::Unicode::Replacement);
        return 1;
      }
      else if constexpr (sizeof(TInputCodeUnit) >= sizeof(char))
      {
  ...
  private:
    ...
    static constexpr auto EncodeReplace(const TEncoding &encoding,
                                        EncodeResult<TInput, TOutput, TState> result) noexcept
    {
      ...
      {
        constexpr TCodeUnit replacements[1] = {static_cast<TCodeUnit>(Krys::Impl::Unicode::Replacement)};
    ...
    template <typename TEncoding, typename TInput, typename TOutput, typename TState>
    static constexpr auto DecodeReplace(const TEncoding &encoding,
                                        DecodeResult<TInput, TOutput, TState> result) noexcept
    {
      ...
      {
        constexpr TCodePoint replacements[1] = {static_cast<TCodePoint>(Krys::Impl::Unicode::Replacement)};
        return Impl::WriteDirect(encoding, replacements, std::move(result));
      }
      else
      {
