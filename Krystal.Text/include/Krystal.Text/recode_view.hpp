...
#ifndef ZTD_TEXT_RECODE_VIEW_HPP
#define ZTD_TEXT_RECODE_VIEW_HPP
...
namespace Krys {
	...
	/// EncodeOne . It may also call `recode_one` to bypass having to do the round-trip through
	...
	class recode_view : public Krys::Ranges::view_base {
	public:
		
		/// @brief The iterator type for this view.
		using iterator = recode_iterator<TFromEncoding, TToEncoding, TRange, _FromErrorHandler, _ToErrorHandler,
			TFromState, TToState>;
		...
		/// @brief Constructs a recode_view from the underlying range.
		///
		/// @param[in] range The input range to wrap and iterate over.
		constexpr recode_view(range_type range) noexcept : recode_view(std::move(range), to_encoding_type {}) {
		}
		...
		constexpr recode_view(range_type range, to_encoding_type __to_encoding) noexcept
		: recode_view(std::move(range), from_encoding_type {}, std::move(__to_encoding)) {
		}
		...
		: recode_view(std::move(range), std::move(__from_encoding), std::move(__to_encoding),
		...
		constexpr recode_view(range_type range, from_encoding_type __from_encoding, to_encoding_type __to_encoding,
...
namespace std { namespace ranges {
	...
	inline constexpr bool enable_borrowed_range<::recode_view<TFromEncoding, TToEncoding, TRange,
