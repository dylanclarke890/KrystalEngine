...
namespace Krys {
	...
	/// @brief A sentinel type that can be used to compare with a transcode_iterator.
	using transcode_sentinel_t = Impl::__encoding_sentinel_t;
	...
	private:
		...
		using TURange                                    = Impl::span_reconstruct_t<_UNonRRange, TRange>;
	...
	public:
		...
		/// @brief Default constructs a transcode_iterator.
		///
		/// @remarks This can only work if the underlying encodings, error handlers, and states can handle default
		/// construction.
		constexpr transcode_iterator() noexcept(NoThrowDefaultConstructible<__base_from_encoding_t>
		...
		template <typename _ArgRange,
			enable_if_t<!SameType<remove_cvref_t<_ArgRange>, transcode_iterator>>* = nullptr>
		constexpr transcode_iterator(_ArgRange&& range) noexcept(
			noexcept(transcode_iterator(std::forward<_ArgRange>(range), to_encoding_type {})))
		: transcode_iterator(std::forward<_ArgRange>(range), to_encoding_type {}) {
		}
		...
		constexpr transcode_iterator(range_type range, to_encoding_type __to_encoding)
		: transcode_iterator(std::move(range), from_encoding_type {}, std::move(__to_encoding)) {
		}
		...
		: transcode_iterator(std::move(range), std::move(__from_encoding), std::move(__to_encoding),
		...
		constexpr transcode_iterator(range_type range, from_encoding_type __from_encoding,
		...
		friend constexpr bool operator==(const transcode_iterator& it, const transcode_sentinel_t&) {
		...
		/// @brief Compares whether or not this iterator has truly reached the end.
		friend constexpr bool operator==(const transcode_sentinel_t& __sen, const transcode_iterator& it) {
			return it == __sen;
		}
	...
	private:
		...
		constexpr void _M_read_one() noexcept {
			...
			using TPivot = Span<_IntermediateCodePoint, MaxCodePoints<_UFromEncoding>>;
			TPivot pivot(__intermediate_storage);
			if constexpr (_IsInputOrOutput) {
				auto result    = transcode_one_into_raw(std::move(__this_input_range), this->from_encoding(),
				...
				auto result    = transcode_one_into_raw(__this_input_range, this->from_encoding(), __cache_view,
				...
				this->TRangeBase::GetValue()
					= Impl::__update_input<TURange>(std::move(result.Input));
			}
			if constexpr (!_IsSingleValueType) {
