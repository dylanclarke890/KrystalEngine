...
namespace Krys
{
  /// @brief A sentinel type that can be used to compare with a RecodeIterator.
  using TRecodeSentinel = Impl::TEncodingSentinel;
  ...
  /// `EncodeOne` . It may also call `RecodeOne` to bypass having to do the round-trip through two encodings,
  /// which an encoding pair that a developer is interested in can use to do the conversion more quickly. The
  /// view presents code units one at a time, regardless of how many code units are output by one decode
  /// operation. This means if, for example, one (1) UTF-16 code unit becomes two (2) UTF-8 code units, it
  /// will present each code unit one at a time. If you are looking to explicitly know each collection of
  ...
  private:
    ...
    using TURange = Krys::Ranges::range_reconstruct_t<TUNonRRange>;
  ...
  public:
    ...
    /// @brief Default constructs a RecodeIterator.
    ///
    /// @remarks This can only work if the underlying encodings, error handlers, and states can handle default
    /// construction.
    constexpr RecodeIterator() noexcept(NoThrowDefaultConstructible<TBaseFromEncoding>
    ...
    /// @brief Copy constructs a RecodeIterator.
    constexpr RecodeIterator(const RecodeIterator &) = default;

    /// @brief Move constructs a RecodeIterator.
    constexpr RecodeIterator(RecodeIterator &&) = default;

    /// @brief Constructs a RecodeIterator from the underlying range.
    ///
    /// @param[in] range The input range to wrap and iterate over.
    constexpr RecodeIterator(range_type range) noexcept(noexcept(RecodeIterator(std::move(range),
                                                                                to_encoding_type {})))
        : RecodeIterator(std::move(range), to_encoding_type {})
    {
    }
    ...
    constexpr RecodeIterator(range_type range, to_encoding_type toEncoding)
        : RecodeIterator(std::move(range), from_encoding_type {}, std::move(toEncoding))
    {
    }
    ...
    /// @param[in] toEncoding The encoding object to call `EncodeOne` or equivalent functionality on.
    constexpr RecodeIterator(range_type range, from_encoding_type fromEncoding, to_encoding_type toEncoding)
        : RecodeIterator(std::move(range), std::move(fromEncoding), std::move(toEncoding),
    ...
    constexpr RecodeIterator(range_type range, from_encoding_type fromEncoding, to_encoding_type toEncoding,
                             from_error_handler_type fromErrorHandler, to_error_handler_type toErrorHandler,
                             from_state_type fromState, to_state_type toState)
    ...
    /// @brief Copy assigns- a RecodeIterator.
    constexpr RecodeIterator &operator=(const RecodeIterator &) = default;

    /// @brief Move assigns a RecodeIterator.
    constexpr RecodeIterator &operator=(RecodeIterator &&) = default;
    ...
    constexpr RecodeIterator operator++(int)
    {
      RecodeIterator _copy = *this;
      ++(*this);
      return _copy;
    }
    ...
    constexpr RecodeIterator &operator++()
    {
    ...
    friend constexpr bool operator==(const RecodeIterator &it, const TRecodeSentinel &)
    {
    ...
    /// @brief Compares whether or not this iterator has truly reached the end.
    friend constexpr bool operator==(const TRecodeSentinel &sentinel, const RecodeIterator &it)
    {
      return it == sentinel;
    }
  ...
  private:
    ...
    constexpr void ReadOne() noexcept
    {
      ...
      using TIntermediate = Span<TIntermediateCodePoint, MaxCodePoints<TUFromEncoding>>;
      TIntermediate pivot(intermediateStorage);
      if constexpr (IsInputOrOutput)
      {
        auto result = Krys::RecodeOneIntoRaw(std::move(thisInputRange), this->FromEncoding(), cacheView,
      ...
      {
        auto result = Krys::RecodeOneIntoRaw(thisInputRange, this->FromEncoding(), cacheView,
                                             this->ToEncoding(), this->FromHandler(), this->ToHandler(),
                                             this->FromState(), this->ToState(), pivot);
        thisCacheEnd = std::to_address(Krys::Ranges::begin(result.Output));
        if constexpr (!IsErrorless)
