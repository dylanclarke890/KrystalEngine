#pragma once

#include "Krystal.Lib/Core/Attributes.hpp"
#include "Krystal.Lib/String/String.hpp"
#include "Krystal.Lib/Types/Set.hpp"
#include "Krystal.Text/Encode/Encode.hpp"
#include "Krystal.Text/Encodings/UTF8.hpp"

namespace Krys
{
  struct UTF8Hash
  {
    using is_transparent = void;

    size_t operator()(utf8_stringview v) const noexcept
    {
      return std::hash<utf8_stringview> {}(v);
    }

    size_t operator()(const utf8_string &s) const noexcept
    {
      return (*this)(utf8_stringview {s});
    }
  };

  struct UTF8KeyEqual
  {
    using is_transparent = void;

    bool operator()(utf8_stringview a, utf8_stringview b) const noexcept
    {
      return a == b;
    }

    bool operator()(const utf8_string &a, utf8_stringview b) const noexcept
    {
      return utf8_stringview {a} == b;
    }

    bool operator()(utf8_stringview a, const utf8_string &b) const noexcept
    {
      return a == utf8_stringview {b};
    }

    bool operator()(const utf8_string &a, const utf8_string &b) const noexcept
    {
      return a == b;
    }
  };

  using InternedString = const utf8_string *;

  class AtomicStringPool
  {
  private:
    Set<utf8_string, UTF8Hash, UTF8KeyEqual> _strings;
    InternedString _empty;

  public:
    AtomicStringPool()
    {
      auto [it, inserted] = _strings.insert(utf8_string {u8""});
      _empty = std::addressof(*it);
    }

    KRYS_NODISCARD InternedString GetOrAdd(utf8_stringview v)
    {
      if (!v.data() || v.size() == 0)
      {
        return _empty;
      }

      if (auto it = _strings.find(v); it != _strings.end())
      {
        return std::addressof(*it);
      }

      // Only allocate when inserting a new unique string.
      auto [insertedIt, wasInserted] = _strings.insert(utf8_string(v));
      return std::addressof(*insertedIt);
    }

    KRYS_NODISCARD InternedString GetOrAdd(const utf8_string &s)
    {
      return GetOrAdd(utf8_stringview {s});
    }

    KRYS_NODISCARD InternedString GetOrAdd(utf8_string &&s)
    {
      if (s.empty())
      {
        return _empty;
      }

      if (auto it = _strings.find(utf8_stringview {s}); it != _strings.end())
      {
        return std::addressof(*it);
      }

      auto [insertedIt, wasInserted] = _strings.insert(std::move(s));
      return std::addressof(*insertedIt);
    }
  };

  class AtomString
  {
  private:
    InternedString _ptr {nullptr};

    static AtomicStringPool &Pool()
    {
      static AtomicStringPool globalPool;
      return globalPool;
    }

  public:
    AtomString() noexcept : _ptr(Pool().GetOrAdd(utf8_stringview {u8""}))
    {
    }

    AtomString(const char8 *str) noexcept : _ptr(Pool().GetOrAdd(utf8_stringview {str}))
    {
    }

    AtomString(utf8_stringview str) noexcept : _ptr(Pool().GetOrAdd(str))
    {
    }

    AtomString(const utf8_string &str) noexcept : _ptr(Pool().GetOrAdd(str))
    {
    }

    AtomString(utf8_string &&str) noexcept : _ptr(Pool().GetOrAdd(std::move(str)))
    {
    }

    utf8_stringview View() const noexcept
    {
      return *_ptr;
    }

    friend bool operator==(AtomString a, AtomString b) noexcept
    {
      return a._ptr == b._ptr;
    }
  };
}