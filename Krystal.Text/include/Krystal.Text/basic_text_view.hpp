...
namespace Krys {
	...
	/// @tparam TRange The range type that will be stored within this basic_text_view and examined using the
	/// iterators, following the `TEncoding` type decoding procedure.
	...
	template <typename TEncoding, typename _NormalizationForm = nfkc,
		typename TRange        = std::basic_string_view<code_unit_t<TEncoding>>,
		typename TErrorHandler = Handlers::DefaultHandler,                        // cf
		typename TState        = DecodeState<remove_cvref_t<TEncoding>> // cf
		>
	...
	public:
		...
		///
		/// @param[in] state The state to use for this code point view.
		/// @param[in] errorHandler The error handler to look at the code points for this code point view.
		///
		/// @remarks Moves the provided `state` in as the "starting point".
		template <typename _ViewErrorHandler>
		constexpr _CodePointView<_ViewErrorHandler> code_points(
			state_type state, _ViewErrorHandler&& errorHandler) const noexcept {
			return _CodePointView<_ViewErrorHandler>(this->Storage, this->_M_encoding,
				std::forward<_ViewErrorHandler>(errorHandler), std::move(state));
		}
		...
		///
		/// @remarks Moves the provided `state` in as the "starting point".
		constexpr _CodePointView<> code_points(state_type state) const noexcept {
			return _CodePointView<>(
				this->Storage, this->_M_encoding, this->_M_error_handler, std::move(state));
		}

		
		/// @brief Returns a view over the code points of this type, decoding "on the fly"/"lazily".
		///
		/// @remarks Copies the stored `state` within the basic_text_view to perform the code point
		/// iteration process.
		constexpr _CodePointView<> code_points() const noexcept {
			return _CodePointView<>(this->Storage, this->_M_encoding, this->_M_error_handler, this->_state);
		}
