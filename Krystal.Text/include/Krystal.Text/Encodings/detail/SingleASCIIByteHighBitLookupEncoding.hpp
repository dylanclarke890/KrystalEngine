...
namespace Krys::Text
{
  template <typename TDerived, EncodingTable::basic_lookup_index_to_code_point_function *TLookupCodePoint,
            ...
            typename TCodePoint = UnicodeCodePoint>
  struct SingleASCIIByteHighBitLookupEncoding
  {
    ...
    {
      return Span<const code_unit, 1>(&Unicode::ASCIIReplacement<code_unit>, 1);
    }
                                    ...
                                    state &s) noexcept
    {
      ...
      {
        size_t lookupIndex = static_cast<size_t>(unit0) - 0x80u;
        const auto maybeCode = TLookupCodePoint(lookupIndex);
        if (maybeCode)
        {
          if constexpr (CallErrorHandler)
          {
                                    ...
                                    state &s) noexcept
    {
      ...
      if (codePoint32 < 0x80)
      {
        ...
        ++inIt;
        *outIt = static_cast<code_unit>(codePoint32);
        ++outIt;
        return TResult(TSubInput(std::move(inIt), std::move(inLast)),
                       TSubOutput(std::move(outIt), std::move(outLast)), s, EncodingError::OK);
      }
      ...
      {
        const auto maybeIndex = TLookupIndex(codePoint32);
        if (maybeIndex)
        {
          if constexpr (CallErrorHandler)
          {
