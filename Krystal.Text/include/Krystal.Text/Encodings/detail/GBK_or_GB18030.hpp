...
namespace Krys::Text
{
  ...
  struct Basic_GBK_GB18030
  {
                                    ...
                                    state &s) noexcept
    {
      ...
      else if ((unit0 >= 0x81 && unit0 <= 0xFE))
      {
        ...
        auto lookupAndWriteOut = [&](std::size_t lookupIndex, auto useRangesTypeValue)
        {
          ...
          constexpr auto toCodePoint = useRanges ? &EncodingTable::GB18030RangesIndexToCodePoint
                                                 : &EncodingTable::GB18030IndexToCodePoint;
          const auto maybeCode = toCodePoint(lookupIndex);
          if (maybeCode)
          {
            if constexpr (CallErrorHandler)
            {
                                    ...
                                    state &s) noexcept
    {
      ...
      auto maybeLookupGBKIndex = EncodingTable::GB18030CodePointToIndex(codePoint32);
      ...
      if constexpr (!IsGBK)
      {
        auto maybeLookupIndex = EncodingTable::GB18030RangesCodePointToIndex(codePoint32);
        if (maybeLookupIndex)
        {
          const size_t lookupIndex = *maybeLookupIndex;
