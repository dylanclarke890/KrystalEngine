...
#pragma endregion
  ...
  KRYS_NODISCARD constexpr Maybe<uint32> ISO_8859_8IndexToCodePoint(size_t lookupIndexPointer) noexcept
  {
    auto lookupIndex = static_cast<const index16>(lookupIndexPointer);
    auto begin = std::ranges::cend(ISO_8859_8_IndexCodePointMap);
    auto end = std::ranges::cend(ISO_8859_8_IndexCodePointMap);
  ...
  KRYS_NODISCARD constexpr Maybe<size_t> ISO_8859_8CodePointToIndex(uint32 lookupCodePoint) noexcept
  {
    auto begin = std::ranges::cend(ISO_8859_8_IndexCodePointMap);
    auto end = std::ranges::cend(ISO_8859_8_IndexCodePointMap);
