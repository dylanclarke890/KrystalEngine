...
#pragma endregion
  ...
  KRYS_NODISCARD constexpr inline std::optional<std::uint_least32_t>
    iso_8859_10_index_to_code_point(std::size_t lookupIndexPointer) noexcept
  {
    const index16 lookupIndex = static_cast<index16>(lookupIndexPointer);

    auto first = Krys::Ranges::cbegin(ISO_8859_10_IndexCodePointMap);
    auto last = Krys::Ranges::cend(ISO_8859_10_IndexCodePointMap);
  ...
  KRYS_NODISCARD constexpr inline std::optional<std::size_t>
    iso_8859_10_code_point_to_index(std::uint_least32_t lookupCodePoint) noexcept
  {
