...
#include "Krystal.Text/Encodings/EncodingTables/Utils/SingleByteHighLookup.hpp"
...
{
#pragma region Table
  constexpr inline const index16_code_point Windows1252IndexCodePointMap[128] = {
...
#pragma endregion
  ...
  KRYS_NODISCARD constexpr Maybe<uint32> Windows1252IndexToCodePoint(size_t lookupIndexPointer) noexcept
  {
    return SingleByteHighIndexToCodePoint(Windows1252IndexCodePointMap, lookupIndexPointer);
  }

  KRYS_NODISCARD constexpr Maybe<size_t> Windows1252CodePointToIndex(uint32 lookupCodePoint) noexcept
  {
    return SingleByteHighCodePointToIndex(Windows1252IndexCodePointMap, lookupCodePoint);
  }
}
