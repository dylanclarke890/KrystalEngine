...
#pragma endregion
  ...
  KRYS_NODISCARD constexpr Maybe<uint32> ISO_8859_16IndexToCodePoint(size_t lookupIndexPointer) noexcept
  {
    auto lookupIndex = static_cast<const index16>(lookupIndexPointer);
    auto begin = std::ranges::cend(ISO_8859_16_IndexCodePointMap);
    auto end = std::ranges::cend(ISO_8859_16_IndexCodePointMap);
  ...
  KRYS_NODISCARD constexpr Maybe<size_t> ISO_8859_16CodePointToIndex(uint32 lookupCodePoint) noexcept
  {
    auto begin = std::ranges::cend(ISO_8859_16_IndexCodePointMap);
    auto end = std::ranges::cend(ISO_8859_16_IndexCodePointMap);
