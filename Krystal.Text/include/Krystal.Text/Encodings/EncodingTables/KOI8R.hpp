...
{
#pragma region Table
  constexpr inline const index16_code_point KOI8RIndexCodePointMap[128] = {
...
#pragma endregion
  ...
  KRYS_NODISCARD constexpr Maybe<uint32> KOI8RIndexToCodePoint(size_t lookupIndexPointer) noexcept
  {
    auto lookupIndex = static_cast<const index16>(lookupIndexPointer);
    auto begin = std::ranges::cend(KOI8RIndexCodePointMap);
    auto end = std::ranges::cend(KOI8RIndexCodePointMap);
  ...
  KRYS_NODISCARD constexpr Maybe<size_t> KOI8RCodePointToIndex(uint32 lookupCodePoint) noexcept
  {
    auto begin = std::ranges::cend(KOI8RIndexCodePointMap);
    auto end = std::ranges::cend(KOI8RIndexCodePointMap);
