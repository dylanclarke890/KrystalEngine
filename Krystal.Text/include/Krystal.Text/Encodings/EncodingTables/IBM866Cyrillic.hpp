...
{
#pragma region Table
  constexpr inline const index16_code_point IBM866CyrillicIndexCodePointMap[128] = {
...
#pragma endregion
  ...
  KRYS_NODISCARD constexpr Maybe<uint32> IMB866CyrillicIndexToCodePoint(size_t lookupIndexPointer) noexcept
  {
    return SingleByteHighIndexToCodePoint(IBM866CyrillicIndexCodePointMap, lookupIndexPointer);
  }

  KRYS_NODISCARD constexpr Maybe<size_t> IMB866CyrillicCodePointToIndex(uint32 lookupCodePoint) noexcept
  {
    return SingleByteHighCodePointToIndex(IBM866CyrillicIndexCodePointMap, lookupCodePoint);
  }
}
