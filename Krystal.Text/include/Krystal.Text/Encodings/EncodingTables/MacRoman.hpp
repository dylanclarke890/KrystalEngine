...
{
#pragma region Table
  constexpr inline const index16_code_point MacRomanIndexCodePointMap[128] = {
...
#pragma endregion
  ...
  KRYS_NODISCARD constexpr Maybe<uint32> MacRomanIndexToCodePoint(size_t lookupIndexPointer) noexcept
  {
    auto lookupIndex = static_cast<const index16>(lookupIndexPointer);
    auto begin = std::ranges::cend(MacRomanIndexCodePointMap);
    auto end = std::ranges::cend(MacRomanIndexCodePointMap);
  ...
  KRYS_NODISCARD constexpr Maybe<size_t> MacRomanCodePointToIndex(uint32 lookupCodePoint) noexcept
  {
    auto begin = std::ranges::cend(MacRomanIndexCodePointMap);
    auto end = std::ranges::cend(MacRomanIndexCodePointMap);
