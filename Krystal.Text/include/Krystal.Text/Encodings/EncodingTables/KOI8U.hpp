...
{
#pragma region Table
  constexpr inline const index16_code_point KOI8UIndexCodePointMap[128] = {
...
#pragma endregion
  ...
  KRYS_NODISCARD constexpr Maybe<uint32> KOI8UIndexToCodePoint(size_t lookupIndexPointer) noexcept
  {
    auto lookupIndex = static_cast<const index16>(lookupIndexPointer);
    auto begin = std::ranges::cend(KOI8UIndexCodePointMap);
    auto end = std::ranges::cend(KOI8UIndexCodePointMap);
  ...
  KRYS_NODISCARD constexpr Maybe<size_t> KOI8UCodePointToIndex(uint32 lookupCodePoint) noexcept
  {
    auto begin = std::ranges::cend(KOI8UIndexCodePointMap);
    auto end = std::ranges::cend(KOI8UIndexCodePointMap);
