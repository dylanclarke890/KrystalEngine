...
#pragma endregion
  ...
  KRYS_NODISCARD constexpr Maybe<uint32> TatarASCIIIndexToCodePoint(size_t lookupIndexPointer) noexcept
  {
    auto lookupIndex = static_cast<const index16>(lookupIndexPointer);
    auto begin = std::ranges::cend(TatarASCIIIndexCodePointMap);
    auto end = std::ranges::cend(TatarASCIIIndexCodePointMap);
  ...
  KRYS_NODISCARD constexpr Maybe<size_t> TatarASCIICodePointToIndex(uint32 lookupCodePoint) noexcept
  {
    auto begin = std::ranges::cend(TatarASCIIIndexCodePointMap);
    auto end = std::ranges::cend(TatarASCIIIndexCodePointMap);
