...
#pragma endregion
  ...
  KRYS_NODISCARD constexpr Maybe<uint32> AtariSTIndexToCodePoint(size_t lookupIndexPointer) noexcept
  {
    auto lookupIndex = static_cast<const index32>(lookupIndexPointer);
    auto begin = std::ranges::cbegin(AtariSTIndexCodePointMap);
    auto end = std::ranges::cend(AtariSTIndexCodePointMap);
  ...
  KRYS_NODISCARD constexpr Maybe<size_t> AtariSTCodePointToIndex(uint32 lookupCodePoint) noexcept
  {
    auto begin = std::ranges::cbegin(AtariSTIndexCodePointMap);
    auto end = std::ranges::cend(AtariSTIndexCodePointMap);
