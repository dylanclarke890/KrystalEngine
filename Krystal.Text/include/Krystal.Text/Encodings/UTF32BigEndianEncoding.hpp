...
namespace Krys::Text
{
  ...
  class UTF32BigEndianEncoding : public Encoding
  {
  public:
    constexpr UTF32BigEndianEncoding() : Encoding {"UTF-32BE"}
    {
    }

    virtual ~UTF32BigEndianEncoding() = default;

    NO_DISCARD constexpr List<byte> GetPreamble() const noexcept override
    {
      return {byte {0x00}, byte {0x00}, byte {0xFE}, byte {0xFF}};
    }
