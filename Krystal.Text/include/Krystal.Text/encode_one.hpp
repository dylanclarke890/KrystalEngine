...
namespace Krys {
	...
	/// @addtogroup ztd_text_encode_one EncodeOne[_into/_to]
	...
	/// @brief Converts a single indivisible unit of work's worth of code points of the given `input` view through
	/// the encoding to code units into the @p output view.
	///
	...
	/// output code units.
	/// @param[in]     output An output_view to write code units to as the result of the EncodeOne operation from
	...
	constexpr auto encode_one_into_raw(TInput&& input, TEncoding&& encoding, TOutput&& output,
	...
	/// @brief Converts a single indivisible unit of work's worth of code points of the given `input` view through
	/// the encoding to code units into the `output` view.
	///
	/// @param[in]     input An input_view to read code points from and use in the EncodeOne operation that will
	/// produce code units.
	/// @param[in]     encoding The encoding that will be used to EncodeOne the input's code points into
	...
	constexpr auto encode_one_into_raw(
		...
		auto __stateful_result = ::encode_one_into_raw(std::forward<TInput>(input),
	...
	constexpr auto encode_one_into_raw(TInput&& input, TEncoding&& encoding, TOutput&& output) {
		...
		return ::encode_one_into_raw(std::forward<TInput>(input),
	...
	constexpr auto encode_one_into_raw(TInput&& input, TOutput&& output) {
	...
	namespace Impl {
		template <typename TInput, typename TEncoding, typename _OutputContainer, typename TErrorHandler,
			typename TState>
		constexpr auto __intermediate_encode_one_to_storage(TInput&& input, TEncoding&& encoding,
			...
			auto result = ::encode_one_into_raw(std::forward<TInput>(input), encoding,
				__intermediate_initial_output, std::forward<TErrorHandler>(errorHandler), state);
		...
		constexpr auto __encode_one_dispatch(
			...
			auto __stateful_result = Impl::__intermediate_encode_one_to_storage(
	...
	/// turning them into a span or string_view if at all recognizable or preferable).
	template <typename TInput, typename TEncoding, typename TOutput, typename TErrorHandler, typename TState>
	constexpr auto encode_one_into(TInput&& input, TEncoding&& encoding, TOutput&& output,
		TErrorHandler&& errorHandler, TState& state) {
		auto __reconstructed_input       = Impl::SpanReconstruct<TInput>(std::forward<TInput>(input));
		auto result                    = ::encode_one_into_raw(std::move(__reconstructed_input),
		...
		return EncodeResult<_ReconstructedResultInput, _ReconstructedResultOutput, TState>(
	...
	constexpr auto encode_one_into(
		...
		auto __stateful_result
			= ::encode_one_into(std::forward<TInput>(input), std::forward<TEncoding>(encoding),
	...
	constexpr auto encode_one_into(TInput&& input, TEncoding&& encoding, TOutput&& output) {
		Handlers::DefaultHandler __handler {};
		return ::encode_one_into(std::forward<TInput>(input), std::forward<TEncoding>(encoding),
	...
	/// code points, by default.
	template <typename TInput, typename TOutput>
	constexpr auto encode_one_into(TInput&& input, TOutput&& output) {
		using TUInput    = remove_cvref_t<TInput>;
		using TCodePoint = ranges::rangeTValue_type_t<TUInput>;
#if ZTD_IS_ON(ZTD_STD_LIBRARY_IS_CONSTANT_EVALUATED)
		if (std::is_constant_evaluated()) {
	...
	constexpr auto encode_one_to(
		...
		if constexpr (_IsVoidContainer && _IsStringable) {
			...
			return Impl::__encode_one_dispatch<false, false, _RealOutputContainer>(
	...
	/// respectively.
	///
	/// @result A stateless_encode_one_result object whose output is of type `_OutputContainer`.
	///
	/// @remarks This function creates a `state` using Krys::CreateEncodeState.
	template <typename _OutputContainer = void, typename TInput, typename TEncoding, typename TErrorHandler>
	constexpr auto encode_one_to(TInput&& input, TEncoding&& encoding, TErrorHandler&& errorHandler) {
		using TUEncoding                = remove_cvref_t<TEncoding>;
		using TState                    = EncodeState<TUEncoding>;
		using _UOutputContainer         = remove_cvref_t<_OutputContainer>;
		using _OutputCodeUnit           = code_unit_t<TUEncoding>;
	...
	constexpr auto encode_one_to(TInput&& input, TEncoding&& encoding) {
		Handlers::DefaultHandler __handler {};
	...
	constexpr auto encode_one_to(TInput&& input) {
		using TUInput    = remove_cvref_t<TInput>;
		using TCodePoint = ranges::rangeTValue_type_t<TUInput>;
#if ZTD_IS_ON(ZTD_STD_LIBRARY_IS_CONSTANT_EVALUATED)
		if (std::is_constant_evaluated()) {
			...
			using TEncoding = default_consteval_code_point_encoding_t<TCodePoint>;
			TEncoding encoding {};
			return ::encode_one_to<_OutputContainer>(std::forward<TInput>(input), encoding);
		}
		else
#endif
		{
	...
	constexpr auto EncodeOne(
		...
		if constexpr (_IsVoidContainer && _IsStringable) {
			using _RealOutputContainer = ::ztd::inline_basic_string<_OutputCodeUnit, __max_units>;
			return Impl::__encode_one_dispatch<true, false, _RealOutputContainer>(
				std::forward<TInput>(input), std::forward<TEncoding>(encoding),
				std::forward<TErrorHandler>(errorHandler), state);
		}
		else {
	...
	constexpr auto EncodeOne(TInput&& input, TEncoding&& encoding, TErrorHandler&& errorHandler) {
		...
		TState state = ::Krys::CreateEncodeState(encoding);
		return ::EncodeOne<_OutputContainer>(std::forward<TInput>(input),
	...
	constexpr auto EncodeOne(TInput&& input, TEncoding&& encoding) {
		Handlers::DefaultHandler __handler {};
		return ::EncodeOne<_OutputContainer>(
			std::forward<TInput>(input), std::forward<TEncoding>(encoding), __handler);
	}
	...
	constexpr auto EncodeOne(TInput&& input) {
		...
		if (std::is_constant_evaluated()) {
			...
			using TEncoding = default_consteval_code_point_encoding_t<TCodePoint>;
			TEncoding encoding {};
			return ::EncodeOne<_OutputContainer>(std::forward<TInput>(input), encoding);
		}
		else
#endif
		{
