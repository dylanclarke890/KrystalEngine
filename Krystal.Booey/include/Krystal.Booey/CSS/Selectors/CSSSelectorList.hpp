...
namespace krys::boo::css
{
  ...
  public:
    ...
    KRYS_NODISCARD const CSSSelector *FirstSelectorComponent() const noexcept
    {
      return _selectors.get();
    }

    KRYS_NODISCARD const CSSSelector *SelectorComponentAt(size_t index) const noexcept
    {
...
#pragma region Static utility functions
    ...
    KRYS_NODISCARD static CSSSelectorList Join(const SmallList<const CSSSelectorList *> &lists) noexcept;
...
#pragma endregion
  ...
  private:
    ...
    KRYS_NODISCARD bool ForEachTagSelector(Functor &&functor, const CSSSelector *selector) const noexcept
    {
      assert(selector);
