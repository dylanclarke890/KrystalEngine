...
namespace krys::boo::dom::detail
{
#pragma region SubtreeRange

  template <typename T, typename TNode>
  concept SubtreeIteratorPolicy = requires(Node *current) {
    { T::IsInclusive } -> ConvertibleTo<bool>;
    { T::Advance(current) } -> SameType<TNode *>;
  };
...
#pragma region ScopedSubtreeRange
  ...
  template <typename T, typename TNode>
  concept ScopedSubtreeIteratorPolicy = requires(TNode *current, Node *stayWithin) {
    { T::IsInclusive } -> ConvertibleTo<bool>;
    { T::ScopedAdvance(current, stayWithin) } -> SameType<TNode *>;
  };
  ...
  private:
    pointer _node;
    match_constness_t<TNode, Node> *_root;
  ...
  public:
    ...
    ScopedSubtreeIterator(pointer node, match_constness_t<TNode, Node> *root) noexcept
...
#pragma region Iterator Policies
  ...
  struct NextSiblingIteratorPolicy
  {
    ...
    KRYS_NODISCARD static TNode *Advance(match_constness_t<TNode, Node> *current) noexcept
    {
      if constexpr (SameType<match_constness_t<TNode, html::HTMLElement>, TNode>)
      {
    ...
    KRYS_NODISCARD static TNode *ScopedAdvance(match_constness_t<TNode, Node> *current,
  ...
  struct AncestorIteratorPolicy
  {
    ...
    KRYS_NODISCARD static TNode *Advance(const Node *current) noexcept
    {
  ...
  struct DescendantIteratorPolicy
  {
    ...
    KRYS_NODISCARD static TNode *ScopedAdvance(const Node *current, const Node *stayWithin) noexcept
    {
...
namespace krys::boo::dom
{
  ...
  using ChildHTMLElementRange =
    detail::ScopedSubtreeRange<detail::NextSiblingIteratorPolicy<html::HTMLElement, false>,
                               html::HTMLElement>;
  using ConstChildHTMLElementRange =
    detail::ScopedSubtreeRange<detail::NextSiblingIteratorPolicy<const html::HTMLElement, false>,
