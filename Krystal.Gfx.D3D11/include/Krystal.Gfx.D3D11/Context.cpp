...
{
  D3D11Context::D3D11Context(NativeHandle windowHandle) : _windowHandle(windowHandle.As<HWND>())
  {
    ::DXGI_SWAP_CHAIN_DESC scd {};
    scd.BufferCount = 1;                                // one back buffer
    scd.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM; // use 32-bit color
    scd.BufferDesc.Width = 1'920;
    scd.BufferDesc.Height = 1'080;
    ...
    viewport.TopLeftX = 0;
    viewport.TopLeftY = 0;
    viewport.Width = 1'920;
    viewport.Height = 1'080;
...
namespace Krys::Gfx
{
  Expected<Unique<IContext>> CreateContext(NativeHandle windowHandle) noexcept
  {
    try
    {
      return Expected<Unique<IContext>>(CreateUnique<D3D11::D3D11Context>(windowHandle));
    }
