...
class StylePropertyCodeGenProperties:
    ...
    def from_json(parsing_context: ParsingContext, key_path: str, name: str, json_value: dict):
        ...
        if "parser-function" in json_value:
            ...
            for entry_name in ["parser-grammar", "skip-parser"]:
                if entry_name in json_value:
...
class NumericLiteral(object):
    ...
    def cpp_unit_type(self):
        if self.kind == NumericLiteral.Kind.NUMBER:
            ...
            return f"CSSUnitType::{self.kind.value}"
    ...
    class Entry:
        def __init__(self, name, *parameter_descriptors):
            ...
            def builtin_schema_type_init(self, parameters):
                ...
                # Fill `results` with mappings from names (e.g. 'value_range' or 'mode') to values (e.g. `CSSRange(0, CSSRange::infinity)` or `HTMLStandardMode`), pulling in default values for unspecified parameters.
                self.results = {}
...
class TermGeneratorMatchAllAnyOrderTerm(TermGenerator):
    ...
    def _generate_lambda(self, *, to: Writer):
        ...
        to.write(
            f"auto ConsumeMatchAllAnyOrder = []({', '.join(lambda_declaration_parameters)}) -> RefPtr<CSSValue> {{"
        )
        with to.indent():
            if self.term.settings_flag:
                to.write(f"if (!state.context.{self.term.settings_flag})")
    ...
    def _generate_lambda_into_builder(self, *, to):
        ...
        to.write(
            f"auto consumeMatchAllAnyOrder = []({', '.join(lambda_declaration_parameters)}) -> std::optional<CSSValueListBuilder> {{"
        )
        with to.indent():
            self._generate_consume_subterm_lambdas(to=to)
...
class TermGeneratorMatchOneOrMoreAnyOrderTerm(TermGenerator):
    ...
    def _generate_lambda(self, *, to):
        ...
        to.write(
            f"auto consumeMatchOneOrMoreAnyOrder = []({', '.join(lambda_declaration_parameters)}) -> RefPtr<CSSValue> {{"
        )
        with to.indent():
            if self.term.settings_flag:
                to.write(f"if (!state.context.{self.term.settings_flag})")
    ...
    def _generate_lambda_into_builder(self, *, to):
        ...
        to.write(
            f"auto consumeMatchOneOrMoreAnyOrder = []({', '.join(lambda_declaration_parameters)}) -> std::optional<CSSValueListBuilder> {{"
        )
        with to.indent():
            self._generate_consume_subterm_lambdas(to=to)
...
class TermGeneratorReferenceTerm(TermGenerator):
    ...
    def _generate_lambda(self, *, to):
        ...
        to.write(
            f"auto Consume{self.term.name.id_without_prefix}Reference = []({', '.join(lambda_declaration_parameters)}) -> RefPtr<CSSValue> {{"
        )
        with to.indent():
            if self.term.settings_flag:
                to.write(f"if (!state.context.{self.term.settings_flag})")
...
class GeneratedSharedGrammarRuleConsumer(SharedGrammarRuleConsumer):
    ...
    def _build_parameters(requires_state):
        parameters = [FunctionParameter("CSSTokenRange &", "range")]
        if requires_state:
...
class GenerateCSSPropertyNames:
    ...
    def _generate_valid_keywords_for_property(self, *, to: Writer):
        ...
        with to.function_block(
            signature="bool CSSProperty::IsKeywordValidForPropertyValues(CSSPropertyId id, CSSValueId keyword, const CSSParserContext &context) noexcept",
        ):
            with to.switch_block(expr="id"):
                for prop, keyword_values, array_name in all_properties_with_values:
...
class GenerateCSSStylePropertiesPropertyNames:
    ...
    def generate_css_style_declaration_property_names_idl(self):
        with open("CSSStyleProperties+PropertyNames.idl", "w") as output_file:
            ...
            self.generation_context.generate_heading(to=writer)
...
class GenerateStyleBuilderGenerated:
    ...
    def _generate_font_property_inherit_value_setter(self, to: Writer, property):
        ...
        to.write(
            f"builderState.{property.codegen_properties.font_description_setter.replace('set', 'setFontDescription', 1)}(WTF::move(inheritedValue));"
        )
    ...
    def generate_style_builder_generated_cpp(self):
        with open("StyleBuilderGenerated.cpp", "w") as output_file:
            ...
            self.generation_context.generate_cpp_required_includes(to=writer, header="StyleBuilderGenerated.h")
...
class GenerateStyleExtractorGenerated:
    ...
    def _generate_style_extractor_generated_cpp_shorthand_value_extractor(self, to: Writer, property):
        to.write(
        ...
        with to.indent():
            to.write(
                f"return extract{property.codegen_properties.shorthand_style_extractor_pattern}Shorthand(extractorState, {property.id_without_prefix_with_lowercase_first_letter}Shorthand());"
            )
        to.write(f"}}")

    def _generate_style_extractor_generated_cpp_shorthand_value_serialization_extractor(self, to: Writer, property):
        to.write(
        ...
        with to.indent():
            to.write(
                f"extract{property.codegen_properties.shorthand_style_extractor_pattern}ShorthandSerialization(extractorState, builder, context, {property.id_without_prefix_with_lowercase_first_letter}Shorthand());"
            )
        to.write(f"}}")

    # Longhand property value getter.

    def _generate_style_extractor_generated_cpp_value_extractor(self, to: Writer, property):
    ...
    def _generate_style_extractor_generated_cpp_value_serialization_extractor(self, to: Writer, property):
        to.write(
    ...
    def generate_style_extractor_generated_cpp(self):
        with open("StyleExtractorGenerated.cpp", "w") as output_file:
            ...
            self.generation_context.generate_cpp_required_includes(to=writer, header="StyleExtractorGenerated.h")
...
class GenerateStylePropertyShorthandFunctions:
    ...
    def generate_style_property_shorthand_functions_h(self):
        with open("StylePropertyShorthandFunctions.hpp", "w") as output_file:
            ...
            self.generation_context.generate_required_header_pragma(to=writer)

            with self.generation_context.namespace("WebCore", to=writer):
                self.generation_context.generate_forward_declarations(to=writer, classes=["StylePropertyShorthand"])
    ...
    def _generate_style_property_shorthand_functions_accessors(
        ...
        for property in self.style_properties.all_shorthands:
            ...
            with to.indent():
                to.write(
                    f"static const CSSPropertyID {property.id_without_prefix_with_lowercase_first_letter}Properties[] = {{"
                )
                ...
                to.write(f"}};")
                to.write(
                    f"return StylePropertyShorthand({property.id}, std::span {{ {property.id_without_prefix_with_lowercase_first_letter}Properties }});"
                )
            to.write(f"}}")
            to.newline()
    ...
    def _generate_style_property_shorthand_functions_matching_shorthands_for_longhand(
        ...
        with to.indent():
            ...
            for longhand, shorthands in sorted(list(longhand_to_shorthands.items()), key=lambda item: item[0].name):
                shorthand_calls = [
                    f"{p.id_without_prefix_with_lowercase_first_letter}Shorthand()"
    ...
    def generate_style_property_shorthand_functions_cpp(self):
        with open("StylePropertyShorthandFunctions.cpp", "w") as output_file:
            ...
            self.generation_context.generate_cpp_required_includes(
                to=writer, header="StylePropertyShorthandFunctions.h"
            )
...
class GenerateCSSPropertyParsing:
    ...
    def generate_css_property_parsing_cpp(self):
        with open("CSSPropertyParsing.cpp", "w") as output_file:
            ...
            self.generation_context.generate_cpp_required_includes(to=writer, header="CSSPropertyParsing.h")
            ...
            with self.generation_context.namespace("WebCore", to=writer):
                self.generation_context.generate_using_namespace_declarations(
                    to=writer, namespaces=["CSSPropertyParserHelpers"]
                )
...
class GenerateStyleInterpolationWrapperMap:
    ...
    def generate_css_property_animation_wrapper_map_cpp(self):
        with open("StyleInterpolationWrapperMap.cpp", "w") as output_file:
            ...
            self.generation_context.generate_cpp_required_includes(to=writer, header="StyleInterpolationWrapperMap.h")
...
class GenerateStyleComputedStyleProperties:
        ...
        with open("StyleComputedStyleProperties.h", "w") as output_file:
            ...
            with writer.namespace(namespace="Krys::HTML"):
                ...
                with writer.indent():
                    ...
                    writer.write(
                        f"ComputedStyleProperties(ComputedStyleProperties& a, ComputedStyleProperties&& b) : ComputedStyleBase {{ a, WTF::move(b) }} {{ }}"
                    )
    ...
    def _generate_getters_inlines_function_definition_logical_side_start_end(
        ...
        to.write(
            f"inline {getter_return_type} ComputedStyleProperties::{prefix.id_without_prefix_with_lowercase_first_letter}{edge}(WritingMode writingMode) const"
        )
        to.write(f"{{")
        with to.indent():
        ...
        to.write(
            f"inline {getter_return_type} ComputedStyleProperties::{prefix.id_without_prefix_with_lowercase_first_letter}{edge}() const"
        )
        to.write(f"{{")
        with to.indent():
        ...
        with open("RenderStyleProperties.h", "w") as output_file:
            ...
            with self.generation_context.namespace("WebCore", to=writer):
                ...
                with writer.indent():
                    ...
                    writer.write(
                        f"RenderStyleProperties(RenderStyleProperties& a, RenderStyleProperties&& b) : RenderStyleBase {{ a, WTF::move(b) }} {{ }}"
                    )
...
class GenerateStyleChangedAnimatablePropertiesGenerated:
    ...
    def generate_style_changed_animatable_properties_generated_cpp(self):
        with open("StyleChangedAnimatablePropertiesGenerated.cpp", "w") as output_file:
            ...
            self.generation_context.generate_cpp_required_includes(
                to=writer, header="StyleChangedAnimatablePropertiesGenerated.h"
            )
