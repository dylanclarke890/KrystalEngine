...
#include "Krystal.Core/Atomics.hpp"
#include "Krystal.Core/Attributes.hpp"
#include "Krystal.Core/Macros.hpp"
#include "Krystal.Core/Numeric.hpp"
#include "Krystal.Core/Types/RefPtr.hpp"
#include "Krystal.Core/Types/TaggedPtr.hpp"
#include <mutex>
...
namespace krys
{
  ...
  {
    KRYS_NON_COPYABLE(ThreadSafeWeakPtrControlBlock);
  ...
  public:
    void AddRefStrong() const noexcept
    {
      ...
      krys_assert_msg(_object, "Object is null");
      ++_strongCount;
    }
    ...
    ThreadSafeWeakPtrControlBlock *AddRefWeak() noexcept
    {
      std::scoped_lock locker {_lock};
      ++_weakCount;
      return this;
    ...
    void SubRefWeak() noexcept
    {
      bool shouldDeleteControlBlock {false};
      {
        ...
        krys_assert_msg(_weakCount, "Weak count is zero");
    ...
    template <typename U>
    KRYS_NODISCARD RefPtr<U> TryCreateStrongReference(const U *maybeInteriorPointer) const noexcept
    {
      std::scoped_lock locker {_lock};
      ...
      if (_object)
      {
        ...
        ++_strongCount;
        return AdoptRefPtr<U>(const_cast<U *>(maybeInteriorPointer));
      }
      return nullptr;
    }
  ...
  private:
    ...
    explicit ThreadSafeWeakPtrControlBlock(
      const RefCountedThreadSafeAndCanMakeWeakPtrThreadSafe<T> *object) noexcept
        : _object(const_cast<T *>(static_cast<const T *>(object)))
    {
    }
  ...
  struct WeakPtrThreadSafeControlBlockPolicy
  {
    ...
    KRYS_ALWAYS_INLINE constexpr static type *AddRef(type *ptr) noexcept
    {
      if (ptr) KRYS_LIKELY
      {
    ...
    KRYS_ALWAYS_INLINE KRYS_NODISCARD constexpr static type *ValidateGetAccess(type *ptr) noexcept
    {
      return ptr;
    }

    KRYS_ALWAYS_INLINE KRYS_NODISCARD constexpr static bool IsValid(type *ptr) noexcept
    {
      return ptr != nullptr;
    }
  };
  ...
  class RefCountedThreadSafeAndCanMakeWeakPtrThreadSafe
  {
    ...
    KRYS_NON_COPYABLE(RefCountedThreadSafeAndCanMakeWeakPtrThreadSafe);
  ...
  public:
    void AddRef() const noexcept
    {
      bool didRefStrongOnly = _bits.transaction(
        [&](uintptr_t &bits)
        {
          ...
          // types.
          krys_assert(!(bits & DestructionStartedFlag));
          bits += RefIncrement;
          return true;
        },
    ...
    void SubRef() const noexcept
    {
      ...
      if (didDerefStrongOnly)
      {
        ...
        {
          krys_assert(_bits.exchangeOr(DestructionStartedFlag) == newStrongOnlyRefCount);
  ...
  private:
    ...
    ThreadSafeWeakPtrControlBlock *ControlBlock(const U &classOrChildClass) noexcept
    {
