...
namespace krys
{
  ...
  template <typename T>
  concept TriviallyMovable = TriviallyMoveAssignable<T> && TriviallyMoveConstructible<T>;
  ...
  /// @brief Detects whether a type is complete. Based on 'Detecting in C++ whether a type is defined, part 3:
  /// SFINAE and incomplete types'
  ...
  constexpr bool AllTrue =
    SameType<std::integer_sequence<bool, true, Bs...>, std::integer_sequence<bool, Bs..., true>>;

  template <typename T>
  concept IsChar = OneOf<T, char, schar, uchar, char8, char16, char32, wchar>;

  template <typename T>
  concept IsCharOrByte = IsChar<T> || SameType<T, byte>;
