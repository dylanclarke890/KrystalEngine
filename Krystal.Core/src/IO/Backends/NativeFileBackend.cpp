...
namespace krys::io
{
  ...
  bool NativeFileBackend::Exists(const Path &path) const noexcept
  {
    ...
    {
      KRYS_LOG_ERROR("Failed to check existence of '{}': {}", path.ToPlatformString(), ex.what());
      KRYS_DEBUG_BREAK();
      return false;
    }
  }
  ...
  bool NativeFileBackend::IsDirectory(const Path &path) const noexcept
  {
    ...
    {
      KRYS_LOG_ERROR("Failed to check if '{}' is a directory: {}", path.ToPlatformString(), ex.what());
      KRYS_DEBUG_BREAK();
      return false;
    }
  }
  ...
  bool NativeFileBackend::IsFile(const Path &path) const noexcept
  {
    ...
    {
      KRYS_LOG_ERROR("Failed to check if '{}' is a file: {}", path.ToPlatformString(), ex.what());
      KRYS_DEBUG_BREAK();
      return false;
    }
  }
                                                    ...
                                                    FileSearchFlags flags) const noexcept
  {
    ...
    {
      KRYS_LOG_ERROR("Failed to search files in '{}': {}", directory.ToPlatformString(), ex.what());
      KRYS_DEBUG_BREAK();
      return {};
    }
  }
