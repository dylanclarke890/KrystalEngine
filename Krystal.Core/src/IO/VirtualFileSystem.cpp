...
#include "Krystal.Core/IO/IFileBackend.hpp"
...
namespace krys::io
{
  ...
  {
    HashSet<Path> seen; // key by RELATIVE path string (virtual)
  ...
  List<Pair<Path, IFileBackend *>> VirtualFileSystem::GetBackends(const Path &path) const noexcept
  {
    ...
    for (const auto &[alias, backend] : _backends)
    {
      if (path.StartsWith(alias))
      {
        ...
        {
          KRYS_LOG_ERROR("Failed to get relative path for backend '{}': {}", alias.ToPlatformString(),
                         ex.what());
          KRYS_DEBUG_BREAK();
          continue;
        }
