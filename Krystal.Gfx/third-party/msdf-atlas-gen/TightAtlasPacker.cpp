...
// disable possible loss of data warnings for entire file
#ifdef _MSC_VER
  #pragma warning(push)
...
namespace msdf_atlas
{
  ...
  TightAtlasPacker::TightAtlasPacker()
      : width(-1), height(-1), spacing(0), dimensionsConstraint(DimensionsConstraint::POWER_OF_TWO_SQUARE),
        scale(-1), minScale(1), unitRange(0), pxRange(0), miterLimit(0), pxAlignOriginX(false),
        pxAlignOriginY(false), scaleMaximizationTolerance(.001)
  {
  }

  int TightAtlasPacker::tryPack(GlyphGeometry *glyphs, int count, DimensionsConstraint dimensionsConstraint,
                                int &width, int &height, double scale) const
  {
    ...
    if (width < 0 || height < 0)
    {
      ...
      switch (dimensionsConstraint)
      {
        case DimensionsConstraint::POWER_OF_TWO_SQUARE:
          dimensions =
            packRectangles<SquarePowerOfTwoSizeSelector>(rectangles.data(), rectangles.size(), spacing);
          break;
